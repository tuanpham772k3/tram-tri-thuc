...
exports.getDocuments = async (req, res) => {
  try {
    ...
    if (search && (typeof search !== "string" || search.trim().length > 100)) {
      return res
        .status(400)
        .json({ success: false, message: "Từ khóa tìm kiếm quá dài." });
    }
    ...
    if (
      sort &&
      !/^(viewCount|downloadCount|createdAt|averageRating):(asc|desc)$/.test(sort)
    ) {
      return res
        .status(400)
    ...
    if (format) {
      ...
      if (!validFormats.includes(format.toLowerCase())) {
        return res
          .status(400)
          .json({ success: false, message: "Định dạng không hợp lệ." });
      }
    ...
    res
      .status(500)
      .json({ success: false, message: "Không thể lấy danh sách tài liệu." });
  }
};
  ...
  try {
    ...
    if (req.user && req.user._id) {
      try {
        ...
        console.log(`Error recording ViewHistory: ${viewError.message}`);
      }
    } else {
      console.log(`No req.user found, skipping ViewHistory for document ${id}`);
    }
...
exports.getDocumentBySlug = async (req, res) => {
  try {
    ...
    if (
      ...
      !(
        req.user &&
        (req.user.role === "admin" || document.uploaderId.equals(req.user._id))
      )
    ) {
      console.log("Access denied to unapproved document", {
        documentId: document._id,
        user: req.user,
      });
      return res
        .status(403)
        .json({ success: false, message: "Tài liệu chưa được duyệt." });
    }
    // Tăng viewCount
    ...
    if (req.user?._id) {
      ...
      console.log(`No authenticated user, skipping ViewHistory for document ${slug}`);
    }

    res.status(200).json({ success: true, data: document });
  } catch (error) {
    console.log("Get document by slug error:", error);
    res.status(500).json({ success: false, message: "Không thể lấy chi tiết tài liệu." });
  }
};
...
exports.downloadDocument = async (req, res) => {
  try {
    ...
    console.log(`Download document error: ${error.message}`, { stack: error.stack });
    if (!res.headersSent) {
      res.status(500).json({ success: false, message: "Không thể tải tài liệu." });
    }
  }
...
exports.uploadDocument = async (req, res) => {
  try {
    ...
    await document.save();
    console.log(`Document uploaded by ${req.user.email}: ${title}`);
    res.status(201).json({
      success: true,
    ...
    console.log(`Upload document error: ${error.message}`, { stack: error.stack });
...
exports.getMyDocuments = async (req, res) => {
  try {
    ...
    if (
      sort &&
      !/^(viewCount|downloadCount|favoriteCount|createdAt):(asc|desc)$/.test(sort)
    ) {
      return res
        .status(400)
        .json({ success: false, message: "Định dạng sắp xếp không hợp lệ." });
    ...
    console.log(`Get my documents error: ${error.message}`, { stack: error.stack });
    res
      .status(500)
      .json({ success: false, message: "Không thể lấy danh sách tài liệu." });
  }
...
exports.getFeaturedDocuments = async (req, res) => {
  try {
    const { search, category, sort, startDate, endDate, dateField, page, limit } =
      req.query;
    ...
    console.log("Get featured documents error:", error);
    res.status(500).json({
      success: false,
...
exports.getRelatedDocuments = async (req, res) => {
  try {
    ...
    if (
      sort &&
      !/^(relevance|viewCount|downloadCount|createdAt):(asc|desc)$/.test(sort)
    ) {
      return res
        .status(400)
        .json({ success: false, message: "Định dạng sắp xếp không hợp lệ." });
    ...
    console.log("Get related documents error:", error);
    res.status(500).json({
      success: false,
...
exports.getVipDocuments = async (req, res) => {
  try {
    console.log("req.user in getVipDocuments:", {
      userId: req.user?._id,
      role: req.user?.role,
    ...
    console.log("Get VIP documents error:", error);
    res
      .status(500)
      .json({ success: false, message: "Không thể lấy danh sách tài liệu VIP." });
  }
};
