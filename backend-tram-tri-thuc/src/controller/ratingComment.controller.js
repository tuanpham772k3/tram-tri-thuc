  ...
  try {
    ...
    if (!content || typeof content !== "string" || content.length > 500) {
      return res.status(400).json({
        success: false,
        message: "Bình luận không được rỗng và tối đa 500 ký tự",
      });
    }
    if (parentCommentId && !Types.ObjectId.isValid(parentCommentId)) {
      return res
        .status(400)
        .json({ success: false, message: "ID bình luận cha không hợp lệ" });
    }
    ...
    if (parentCommentId) {
      ...
      if (!parentComment || parentComment.isDeleted) {
        return res.status(404).json({
          success: false,
          message: "Bình luận cha không tồn tại hoặc đã bị xóa",
        });
      }
      if (parentComment.documentId.toString() !== documentId) {
        return res
          .status(400)
          .json({ success: false, message: "Bình luận cha không thuộc tài liệu này" });
      }
    }
    ...
    console.log(`Comment created by ${req.user.email} for document ${documentId}`);
    res
      .status(201)
      .json({ success: true, message: "Bình luận thành công", data: comment });
  } catch (error) {
...
exports.getCommentsByDocument = async (req, res) => {
  try {
    ...
    if (sortBy && !["createdAt"].includes(sortBy)) {
      return res
        .status(400)
        .json({ success: false, message: "Sắp xếp chỉ hỗ trợ createdAt" });
    }
    if (sortOrder && !["asc", "desc"].includes(sortOrder)) {
      return res
        .status(400)
    ...
    res
      .status(500)
      .json({ success: false, message: "Không thể lấy danh sách bình luận" });
  }
};
...
exports.updateComment = async (req, res) => {
  try {
    ...
    if (!isAdmin && comment.userId.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ success: false, message: "Bạn không có quyền cập nhật bình luận này" });
    }
    ...
    console.log(`Comment updated by ${req.user.email} for comment ${commentId}`);
    res
      .status(200)
      .json({ success: true, message: "Cập nhật bình luận thành công", data: comment });
  } catch (error) {
...
exports.deleteComment = async (req, res) => {
  try {
    ...
    console.log(`Comment deleted by ${req.user.email} for comment ${commentId}`);
...
exports.reportComment = async (req, res) => {
  try {
    ...
    if (!comment || comment.isDeleted) {
      return res
        .status(404)
        .json({ success: false, message: "Bình luận không tồn tại hoặc đã bị xóa" });
    }

    if (comment.isReported) {
      return res
        .status(400)
        .json({ success: false, message: "Bình luận này đã được báo cáo" });
    }

    comment.isReported = true;
    await comment.save();

    console.log(`Comment ${commentId} reported by user ${req.user.email}`);
    res.status(200).json({ success: false, message: "Báo cáo bình luận thành công" });
  } catch (error) {
    console.log(`Report comment error: ${error.message}`);
