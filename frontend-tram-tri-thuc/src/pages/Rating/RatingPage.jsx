...
const RatingPage = () => {
  ...
  const hasVipAccess =
    userInfo && (userInfo.isVip === "active" || userInfo.role === "admin");

  // Debounce tìm kiếm
  const debouncedSearch = useMemo(
    () => debounce((value) => setSearchTerm(value), 50),
    []
  );
  ...
  const handleDocumentSelect = (docId) => {
    ...
    dispatch(
      fetchRatingsByDocument({ documentId: docId, params: { page: 1, limit: 10 } })
    );
  ...
  const handleNewRating = ({ documentId, score, review }) => {
    dispatch(createRating({ documentId, score, review })).then((result) => {
            ...
            <div className="flex items-center space-x-4 flex-wrap gap-2">
              ...
              <select
                ...
                className={`px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 ${
                  accessLevelFilter === "vip" ? "opacity-50 cursor-not-allowed" : ""
                }`}
                  ...
                  <p className="text-gray-500 text-sm text-center py-8">
                    {accessLevelFilter === "vip"
                      ...
                      : `Không có tài liệu trong danh mục ${
                          categories.find((cat) => cat.slug === filterCategory)?.name ||
                          ""
                        }`}
                  </p>
                ) : (
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-4 text-sm text-gray-600 mb-4">
                          <div className="flex items-center space-x-1">
                            ...
                            <span>{selectedDocument.uploaderId?.name || "Ẩn danh"}</span>
                          </div>
                          <div className="flex items-center space-x-1">
                          ...
                          {accessLevelFilter !== "vip" && (
                            <div className="flex items-center space-x-1">
                              ...
                              <span>{selectedDocument.categoryId?.name}</span>
                            </div>
                          )}
                        </div>
                        <div className="grid grid-cols-3 gap-4 text-center">
                          <div className="bg-blue-50 p-3 rounded-lg">
                            <div className="text-2xl font-bold text-blue-600">
                              {selectedDocument.averageRating?.toFixed(1) || 0}
                            </div>
                          ...
                          <div className="bg-green-50 p-3 rounded-lg">
                            ...
                            <div className="text-xs text-gray-600">Tải về</div>
                          </div>
