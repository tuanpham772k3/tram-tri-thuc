...
const DocumentApproval = () => {
  ...
  const fetchDocuments = async () => {
    try {
      setLoading(true);
      const response = await customAxios.get("/admin/document", {
  ...
  const handleApprove = async (docId) => {
    try {
      const response = await customAxios.put(`/admin/approveDocument`, {
        documentId: docId,
      });
      if (response.data.success) {
  ...
  const handleReject = async (docId) => {
    try {
      const response = await customAxios.put(`/admin/rejectDocument`, {
        documentId: docId,
      });
      if (response.data.success) {
        await fetchDocuments();
